...
describe ESM::Command::System::Decline, category: "command" do
  ...
  describe "#execute" do
    ...
    it "should run (Valid UUID)" do
      embed = nil
      event = CommandEvent.create("!decline #{request.uuid_short}", user: user_2, channel_type: :dm)
    ...
    it "should run (Invalid UUID)" do
      ...
      event = CommandEvent.create("!decline #{request.uuid_short}", user: user_1, channel_type: :dm)
